...
def convert_to_webp(src, dest):
    ...
    """
    # Готовый файл повторно не пересобираем, но декодируем: битый webp мог
    # оказаться на диске не из-за парсера (ручная правка, сбой файловой
    # системы, кривая синхронизация папки), и тогда его надо пересобрать,
    # а не молча оставить в базе ссылку на нечитаемую картинку.
