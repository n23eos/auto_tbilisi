...
def strip_frontmatter(text):
    ...
    Границы ищем ПОСТРОЧНО. Поиск по подстроке (text.split("---")) считал концом
    шапки любое тире в тексте: шапки в baza-znaniy заполняются свободным текстом
    по-русски, и «тема: Цены --- обновлено» обрезала файл прямо внутри значения —
    терялся заголовок # Тема, а в тексте оставался огрызок «---».

    Незакрытая шапка возвращается как есть: молча съесть остаток файла хуже,
    чем оставить лишние строки, которые видно глазами.
    """
    lines = text.split("\n")
