...
1. **`/resend` без лимита.** `SELECT ... WHERE delivery_status='pending'` без
...
2. **Нет ретрая на исходящий запрос к Telegram.** Один 429 или 5xx теряет ответ
   навсегда, потому что `processed_updates` пишется до обработки. Достаточно
   одного повтора с коротким backoff.

3. **Молчание на опечатку в админ-команде.** `/zayvki` не даёт вообще ничего —
...
4. **`ADMIN_IDS` парсится молча.** `"777 888"` через пробел или `@username`
...
5. **Нет `CHECK` на `leads.status` и `delivery_status`.** Неизвестный статус
   отрисуется как `undefined` без кнопок — ещё один способ застрять.

6. **`splitMessage` может разрезать HTML-сущность** посередине, если в разделе
...
10. **`compatibility_date = 2026-08-01`** выше версии локального workerd, в
